...
def analyze_all_layers(
    ...
    def beautify(fig):
        # 获取y轴标签并设置为position1-position6格式
        y_labels = fig.get_yticklabels()
        n_positions = len(y_labels)
        position_labels = [f'position{i+1}' for i in range(n_positions)]
        ...
        x_labels = fig.get_xticklabels()
        kept_labels = x_labels[::2]
        new_labels = ['' for _ in x_labels]
        new_labels[::2] = kept_labels
        
        # 设置y轴标签并应用小号字体
        fig.set_yticklabels(position_labels, fontsize=8)  # 减小字体至8pt
        fig.set_xticklabels(new_labels, fontsize=6)  # 减小字体至8pt
        
        # 设置y轴名称（使用小号字体）
        fig.set_ylabel('Token', fontsize=12)  # 轴标题稍大为9pt
        fig.set_xlabel('Layer', fontsize=12)  # 轴标题稍大为9pt

    # Pearson correlation trends
    converted_df = df.pivot(index='position', columns='layer', values='pearson')
    fig = sns.heatmap(data=converted_df, annot=False, cmap='GnBu')
    beautify(fig)
    savefig(fig, out_path, 'pearson_trend', 'svg')
