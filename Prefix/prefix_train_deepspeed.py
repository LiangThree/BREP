...
def train(
    ...
    model, config = load_RED_model(model_path=model_path, op_position=op_position, layer_type=layer_type)

    model.config = config
    model.global_weight = torch.tensor(weight, dtype=torch.float32)
    ...
    combined_data = combined_data.filter(lambda x:x["prompt_length"] <= MAX_LENGTH and x["text_length"] <= MAX_LENGTH)
    ...
    deepspeed_config = {
        ...
        "activation_checkpointing": {
            "partition_activations": True,  # 分区保存激活值
            "contiguous_memory_optimization": True,  # 连续内存优化
            "cpu_checkpointing": True  # 将激活值卸载到CPU
        }
    }
